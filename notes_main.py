...
def add_note(): #1 функция добавляет заметку
...
def save_note(): #1 функция сохраняет заметку
...
def del_note(): #1 функция удаляет заметку
    if ui.list_notes.selectedItems():
        key = ui.list_notes.selectedItems()[0].text()
        del notes[key]
        ui.list_notes.clear()
...
def del_tag():
    if ui.list_notes.selectedItems():
        ...
        print("Заметка для удаления не выбран!")
...
def search_tag():
    ...
    if ui.button_tag_search.text() == "Искать" and tag:
        ...
        ui.button_tag_search.setText("Сбросить")
        ui.list_notes.clear()
        ui.list_tags.clear()
        ui.list_notes.addItems(notes_filtered)
        print(ui.button_tag_search.text())
    elif ui.button_tag_search.text() == "Сбросить":
        ui.field_tag.clear()
        ui.list_notes.clear()
        ui.list_tags.clear()
        ui.list_notes.addItems(notes)
